...
function RoleRoute({
  ...
  useEffect(() => {
    if (!isLoading && isLoggedIn && !allowed.includes(role)) {
      const requiredRole =
        allowed.length === 1 ? allowed[0] : allowed.join(" or ");
      showToast(
        `🚫 Access Denied. ${requiredRole.charAt(0).toUpperCase() + requiredRole.slice(1)} Only`,
      );
  ...
  if (!allowed.includes(role)) {
    return (
      <div className="card max-w-xl text-center py-12 border border-black/10 shadow-card">
        ...
        <div className="text-slate-700 font-semibold mb-2 text-xl">
          Access Denied
        </div>
        <div className="text-slate-500">
          This page is restricted to{" "}
          <span className="font-medium capitalize">{allowed.join(" or ")}</span>{" "}
          users only.
        </div>
      </div>
    );
...
export default function App() {
  return (
    <Routes>
      ...
      <Route
        path="/about-daing"
        element={<Navigate to="/about-daing/espada" replace />}
      />
      <Route
        path="/about-daing/:slug"
        element={
          <Layout>
            <AboutDaingTypePage />
          </Layout>
      ...
      <Route
        path="/publications"
        element={<Navigate to="/publications/local" replace />}
      />
      <Route
        path="/publications/local"
        element={
          <Layout>
            <PublicationsPage type="local" />
          </Layout>
        }
      />
      <Route
        path="/publications/foreign"
        element={
          <Layout>
            <PublicationsPage type="foreign" />
          </Layout>
        }
      />
    </Routes>
